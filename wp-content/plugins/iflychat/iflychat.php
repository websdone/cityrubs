...
define('IFLYCHAT_PLUGIN_VERSION', 'WP-4.6.0');
...
{   $current_user =  wp_get_current_user();
  ...
  if (iflychat_get_user_id() && iflychat_get_user_name()) {
    ...
    if(iflychat_chatcamp_check()) {
        $options['body'] = json_encode(array('id' => $data['user_id']));
        $options['headers'] = array('Content-Type' => 'application/json', 'x-api-key' => $data['api_key']);
        $result = wp_remote_head(CHATCAMP_EXTERNAL_A_HOST . ':' . CHATCAMP_EXTERNAL_A_PORT . '/api/1.0/users.get', $options);
        if (!is_wp_error($result) && $result['response']['code'] == 200) {
            ...
            $options['body'] = json_encode(iflychat_chatcamp_process_user_data($data));
            $result = wp_remote_head(CHATCAMP_EXTERNAL_A_HOST . ':' . CHATCAMP_EXTERNAL_A_PORT . '/api/1.0/users.create', $options);
            if (!is_wp_error($result) && $result['response']['code'] == 200) {
                $result = json_decode($result['body']);
                if (is_user_logged_in()) {
                    $_SESSION['token'] = $result->access_token;
...
Wordpress Version: <?php global $wp_version; echo esc_html( $wp_version ); ?>
...
Plugins: <?php
    ...
    $dashboardUrl = "https://dashboard.chatcamp.io";
    if(!iflychat_chatcamp_check()) {
...
function iflychat_validate_fields(){
    ...
    if(iflychat_chatcamp_check() || (strlen($app_id) == 36 && $app_id[14] == '4')){
        return true;
...
function iflychat_chatcamp_check() {
    ...
    $app_id = iflychat_get_option('iflychat_app_id');
    if(!empty($app_id)) {
        if(strpos($app_id, '-') === false && strlen($app_id) == 19) {
            return true;
        }
    }
    return false;
