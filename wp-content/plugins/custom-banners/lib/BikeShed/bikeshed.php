...
if (!class_exists('Custom_Banners_GoldPlugins_BikeShed'))
{
	class Custom_Banners_GoldPlugins_BikeShed
	{
		...
		{
			//RWG: only enqueue scripts and styles on Custom Banners pages
...
<?php
		...
		function grouped_select($options = array())
		{
			if (!is_array($options)) {
				$options = array('name' => $options);
			}
...
?>	
			<div class="bikeshed bikeshed_select">
				<div class="select_wrapper">
					<select name="<?php echo $select_name; ?>" id="<?php echo $select_id; ?>" <?php echo ($options['disabled'] ? 'disabled="disabled"' : ''); ?>>	
						<?php foreach($options['options'] as $group_slug => $group_options): ?>
							<?php $first_item = array_shift($group_options); ?>
							<optgroup label="<?php echo $first_item ?>">
								<?php foreach($group_options as $value => $name): ?>
								<?php
									$disabled = is_array($name) && isset($name['disabled']) ? $name['disabled'] : false;
									$name = is_array($name) ? $name['name'] : $name;
								?>
								<option value="<?php echo $value; ?>" <?php if($disabled): echo 'disabled="disabled"'; endif; ?> <?php if($options['value'] == $value): echo 'selected="selected"'; endif; ?>><?php echo $name; ?></option>
								<?php endforeach; ?>
							</optgroup>
						<?php endforeach; ?>
					</select>
...
?>	
			<div class="bikeshed bikeshed_typography">
				...
				<div class="color_picker_wrapper">
					...
					<input type="text" name="<?php echo $font_color_name; ?>" id="<?php echo $font_color_id; ?>" class="wp-color-picker" value="<?php echo $values['font_color']; ?>" />
					<?php else: ?>
...
<?php	
		...
		/* Generic function to output non-input content, i.e., text or a message
		 * Requires only 2 keys in $options: 
		 *    string	label		The text of the label
		 *    string	message		The text to echo where the input would usually go. May contain HTML.
		 *    string	class		Optional, HTML class with which to wrap the message.
...
} // endif class_exists('Custom_Banners_GoldPlugins_BikeShed')