...
class GhostPool_Addons_Manager {

	/**
	 * @var GhostPool_Addons_Manager The single instance of the class
	 * @since 1.0.0
	 */
	...
	/**
	 * Main GhostPool_Addons_Manager Instance
	 *
	 * Ensures only one instance of GhostPool_Addons_Manager is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @see GhostPool_Addons_Manager()
	 * @return GhostPool_Addons_Manager - Main instance
	 */
	...
	function __construct() {
		...
		add_action( 'wp_ajax_ghostpool_do_plugin_action', array( $this, 'do_plugin_action' ) );

		// run code on class init
		do_action( 'ghostpool_addons_manager_init' );
	...
	public function do_plugin_action() {
		...
		check_ajax_referer( 'ghostpool_addons_action', 'security' );
	...
	function do_plugin_update( $slug ) {
		...
		$status['error'] = esc_html__( 'The plugin does not have an update.', 'aardvark' );
		wp_send_json_error( $status );
	...
	function install_child_theme( $slug ) {
		if ( empty( $this->plugins[ $slug ] ) ) {
			wp_send_json_error( array( 'error' => esc_html__( 'We cannot find this child theme.', 'aardvark' ) ) );
		}
		...
		if ( ! current_user_can( 'install_themes' ) ) {
			$status['error'] = esc_html__( 'You do not have the permissions to install themes.', 'aardvark' );
		...
		if ( isset( $plugin_config['addon_type'] ) && $plugin_config['addon_type'] === 'child_theme' ) {
			...
			if ( $this->is_child_theme_installed() ) {
				...
				if ( $this->is_child_theme_active() ) {
					$status['status']      = 'gp-active gp-addons-disabled';
					$status['status_text'] = esc_html__( 'Active', 'aardvark' );
					$status['action_text'] = esc_html__( 'Child theme installed and active', 'aardvark' );
					$status['action']      = 'no_action';
				} else  {
					$status['status']      = 'gp-inactive';
					$status['status_text'] = esc_html__( 'Inactive', 'aardvark' );
					$status['action_text'] = esc_html__( 'Activate child theme', 'aardvark' );
					$status['action']      = 'enable_child_theme';
				}
			} else{
				$status['status']      = 'gp-not-installed';
				$status['status_text'] = esc_html__( 'Not installed', 'aardvark' );
				$status['action_text'] = esc_html__( 'Install child theme', 'aardvark' );
				$status['action']      = 'install_theme';

				if ( ! current_user_can( 'install_themes' ) ) {
					$status['status']         = 'gp-not-installed gp-addons-disabled';
					$status['action_text']    = esc_html__( 'Permissions needed to install child themes. Contact site administrator.', 'aardvark' );
			...
			if ( $this->is_plugin_installed( $plugin_slug ) ) {
				if ( $this->does_plugin_have_update( $plugin_slug ) ) {
					$status['status']         = 'gp-has-update';
					$status['status_text']    = esc_html__( 'Needs update', 'aardvark' );
					$status['action_text']    = esc_html__( 'Update plugin', 'aardvark' );
					$status['action']         = 'update_plugin';
				}
				elseif ( $this->is_plugin_active( $plugin_slug ) ) {
					$status['status']         = 'gp-active';
					$status['status_text']    = esc_html__( 'Active', 'aardvark' );
					$status['action_text']    = esc_html__( 'Deactivate plugin', 'aardvark' );
					$status['action']         = 'disable_plugin';
				} else  {
					$status['status']         = 'gp-inactive';
					$status['status_text']    = esc_html__( 'Inactive', 'aardvark' );
					$status['action_text']    = esc_html__( 'Activate plugin', 'aardvark' );
				...
				$status['status']         = 'gp-not-installed';
				$status['status_text']    = esc_html__( 'Not Installed', 'aardvark' );
				$status['action_text']    = esc_html__( 'Install plugin', 'aardvark' );
				$status['action']         = 'install_plugin';

				if ( ! current_user_can( 'install_plugins' ) ) {
					$status['status']         = 'gp-not-installed gp-addons-disabled';
					$status['action_text']    = esc_html__( 'You do not have permission to install plugins. Contact the site administrator.', 'aardvark' );
					$status['action']         = 'contact_network_admin';
				}
	...
	function do_plugin_install( $slug, $echo = true ) {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			$status['error'] = esc_html__( 'You do not have permission to install plugins.', 'aardvark' );
			if ( $echo ) {
				wp_send_json_error( $status );
			} else {
				return $status;
	...
	function do_plugin_deactivate( $slug ) {
		...
		if ( empty( $this->plugins[ $slug ] ) ) {
			$status['error'] = esc_html__( 'We have no data about this plugin.', 'aardvark' );
			wp_send_json_error( $status );
		}
