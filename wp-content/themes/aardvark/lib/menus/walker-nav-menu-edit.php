 ...
 * @since 0.1.0
 */
class GhostPool_Menu_Item_Custom_Fields_Walker extends Walker_Nav_Menu_Edit {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $tag = version_compare( $GLOBALS[ 'wp_version' ], '4.6.1' ) > 0 ? 'fieldset' : 'p';
        $output .= preg_replace(
            '/(?=<' . $tag . '[^>]+class="[^"]*field-move)/', $this->get_fields( $item, $depth, $args ), $item_output
        );
    }
